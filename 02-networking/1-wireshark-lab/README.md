...
Resolving a username to a full display name requires a different protocol than Kerberos — **SAMR (Security Account Manager Remote Protocol)**, which carries structured account detail fields over RPC-over-SMB.
...
Together, these five findings give a SOC team everything needed to physically locate the infected machine, identify its user, and begin containment — the full chain from network alert to actionable identity.
...
This kind of correlation — an outbound connection with no matching prior DNS query — is exactly the sort of check a SIEM should automate rather than rely on manual review. A viable detection rule: **alert when a host makes an outbound connection to an external IP with zero corresponding DNS query in the preceding time window.**
...
In practice, this filter surfaced mostly unrelated TCP retransmission chatter from a separate, healthy TLS/CDN connection — not a clean view of "the interesting stuff." This filter doesn't strip out retransmission noise, which can dominate the view when one connection experienced transmission problems during capture.
...
Analysis of the TCP three-way handshake (`tcp.flags.syn==1`) showed the C2 channel used a single persistent connection rather than repeated new connections, with HTTP Keep-Alive maintaining the session.
