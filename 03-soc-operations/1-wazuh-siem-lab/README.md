...
Suricata is installed on Ubuntu. HOME_NET was configured for the Ubuntu host/network being monitored, with packet capture bound to the correct interface. Wazuh agent picks up Suricata logs via the `<localfile>` block and sends them to the Wazuh manager.
...
*Figure 1 — Wazuh Threat Hunting → Events, filtered to* `manager.name: kali`*. Repeated* `Suricata: Alert - GPL ICMP_INFO PING *NIX` *entries confirm the Suricata → Wazuh JSON decoding pipeline is live.*
...
This confirms that the system works: Suricata detects, the Wazuh agent picks up Suricata reports and sends them to the Manager, which generates an alert on the dashboard with a rule ID.
...
Wazuh already has rules 550 and 554 that detect when files are modified or newly created. We create our own rules that specifically watch for those events under /root. Each custom rule has its own ID. The VirusTotal integration is configured to watch for those custom rule IDs, so when one of those rules fires, Wazuh can send the file's hash to VirusTotal for checking.
...
This lab demonstrates the deployment and operation of a multi-host Wazuh environment, including endpoint enrollment, Suricata network intrusion detection, File Integrity Monitoring, VirusTotal integration, and automated Active Response. The lab produced observable evidence at each stage of the detection pipeline, from network events and file creation to threat intelligence analysis and automated file removal. These capabilities demonstrate practical experience with SIEM monitoring, security alert analysis, IOC investigation, and automated incident response.
